...
このプロジェクトは、画像のNSFW（Not Safe For Work）判定を行う2段階構成の
モデルを実装しています：

- Stage A: 画像特徴抽出（256次元埋め込み）
          NudeNet YOLOv8 backbone + GAP + 直交射影
...
■ 分類パイプライン
    ...
    [画像] → [NudeNet YOLOv8 Backbone] → [GAP + Orthogonal Projection] → [256-dim 埋め込み]
                                                                                  ↓
    [256-dim 埋め込み] → [BatchNorm + Dense(tanh/gelu)] x 1-2 → [Dense(sigmoid)] → [NSFW確率]
...
分類器に SFW と誤認させる摂動生成ネットワークを学習します。

本手法は FGSM・PGD・Carlini & Wagner・DeepFool のいずれとも異なり、
フィードフォワード型の生成モデルが一回のフォワードパスで摂動を出力する
独自設計です。
...
■ 損失関数
    ...
    L_perceptual        人間の視覚に近い多層 VGG 特徴比較           λ=1.0
...
分類器と射影層がまだ無い場合は、先に Stage A → B を完了し、CLIP 射影層を
生成してください：
